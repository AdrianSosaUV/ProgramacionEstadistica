class BCI():

    def __init__(self, adyacencia):
        self.ady = adyacencia
        self.ini = inicio 
        self.des = destino
        self._init_grafo(-1)

    def _init_grafo(self, inicio):
        self.encontrado = [False for n in self.ady]
        self.procesado = [False for n in self.ady]
        self.padre = [-1 for n in self.ady]
        self.inicio = inicio

    def profundidad(self, inicio, destino):
        """Usar busqueda en profundidad desde inicio a todo el grafo"""
        self._init_grafo(inicio)
        q = [inicio]
        self.encontrado[inicio] = True
        while q:
            v = q.pop()
            self.procesado[v] = True
            for vecino in self.ady[v]:
                if not self.encontrado[vecino]:
                    q.append(vecino)
                    self.encontrado[vecino] = True
                    self.padre[vecino] = v
        if self.padre[destino] == -1 or self.inicio == -1:
            return None
        self.camino = [destino,]
        p = destino
        while p != self.inicio:
            self.camino.append(self.padre[p])
            p = self.padre[p]
        """Devuelve el camino entre los vertices inicio y destino"""
        return self.camino





adyacencia = [
    [1, 3, 4],
    [0, 2, 3, 4, 5],
    [1, 4, 5],
    [0, 1],
    [0, 1, 2],
    [1,2]]

inicio = 0
destino = 5
g = BCI(adyacencia)
camino = g.profundidad(inicio, destino)
camino.reverse()
[print(n) for n in camino] if camino else print("no encontrado")
#if camino:
#    print("el camino encontrado es :")
#    for n in camino:
#        print(n) 
#else:
#    print("no encontrado")

